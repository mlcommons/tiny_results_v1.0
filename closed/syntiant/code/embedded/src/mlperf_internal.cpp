...
arg_claimed_t ee_profile_parse(char *command) {
  ...
  if (strncmp(command, "profile", EE_CMD_SIZE) == 0) {
    ...
    th_printf("m-model-[%s]\r\n", TH_MODEL_VERSION);
  } else if (strncmp(command, "toggle", EE_CMD_SIZE) == 0) {
    th_printf("Toggling GPIO on NDP120\r\n");
    toggle_gpio();
  } else if (strncmp(command, "get_msg", EE_CMD_SIZE) == 0) {
    th_printf("Fetching message from NDP120\r\n");
    th_get_msg();
    ...
    ee_infer(n, w);
  } else if (strncmp(command, "send_zeros", EE_CMD_SIZE) == 0) {
...
void ee_infer(size_t n, size_t n_warmup) {
  //printf("jhdbg: in ee_infer[1]. g_buff_pos = %d\r\n", g_buff_pos);
