...
This file downloads the speech command dataset and runs the necessary 
...
def get_training_data(Flags):
  ...
  # ds_test = ds_test.shuffle(test_shuffle_buffer_size)
    ...
    # Remove the next three lines if network is channel first
    ds_train_specs = ds_train_specs.map(lambda x, y : (tf.transpose(x, perm=[0, 3, 1, 2]), y))
    ds_val_specs = ds_val_specs.map(lambda x, y : (tf.transpose(x, perm=[0, 3, 1, 2]), y))
    ds_test_specs = ds_test_specs.map(lambda x, y : (tf.transpose(x, perm=[0, 3, 1, 2]), y))
