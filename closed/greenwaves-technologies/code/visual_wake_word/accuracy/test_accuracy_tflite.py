...
def main():
    ...
    parser.add_argument('--tflite_model', type=str, default='../model/vww_96_int8.tflite', 
    ...
    with open(ANNOTATION_FILE_PATH) as csv_file:
        ...
        for index, row in tqdm(enumerate(csv_reader), total=n_files):
            ...
            if os.path.isfile(file_name):
                ...
                dat = np.fromfile(os.path.join(BASEPATH, entry), dtype=np.uint8).reshape(input_details[0]['shape']).astype(np.float32) / 255

                if input_details[0]['dtype'] == np.float32:
                    interpreter.set_tensor(input_details[0]['index'], dat)
                elif input_details[0]['dtype'] == np.int8:
                    dat_q = np.array(dat / scale + zero_point, dtype=np.int8) # should match input type in quantize.py
